...
class NmCourtsSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        yield scrapy.FormRequest(url=url, headers=self.headers, method='POST', formdata=data, callback=self.parse_accept,
                                 dont_filter=True)
    ...
    def parse_case_lookup(self, response):
        ...
        if current_input.get('court_type'):
            data = {
                'formids': 'csrfToken,If_2,If_3,partyName,driversLicense,dlState,If_4,dob,yearOnlyDob,caseNumberPartialSearch,courtTypeSelection,dol,caseCategory,dateSearch,dateType,monthFromSelection,dol2,yearFrom,monthToSelection,dol3,yearTo,results,Submit',
                'component': 'nameSearchForm',
                'page': 'NameSearch',
    ...
    def parse_search(self, response):
        for row in response.xpath('//tr[contains(@id,"informal")]'):
            item=dict()
            item['case_number']=''.join([text.strip() for text in row.xpath('./td[@class="fullCaseNumberColumnValue"]//text()').getall() if text])
            ...
            item['filing_date']=''.join([text.strip() for text in row.xpath('./td[@class="filingDateColumnValue"]//text()').getall() if text])
